 ...
 * implemented all the common method that simple segment and Complex segment
...
public abstract class ASegment implements ISegment {
    ...
    public ASegment(Reader input, JcsegTaskConfig config, ADictionary dic)
            throws IOException {
        this.config = config;
        this.dic = dic;
    ...
    public IWord next() throws IOException {
        /*
         * @Note: check and get the token directly from the word pool if word
         * pool is available. changed wordPool to IHashQueue for the same Word
         * in wordPool the start position of the word will be the last one
         * 
         * @added: 2014-04-11
         */
        if (wordPool.size() > 0)
        ...
        while ((c = readNext()) != -1) {
                ...
                while (cjkidx < chars.length) {
                    ...
                    if (CNNMFilter.isCNNumeric(chars[cjkidx]) > -1
                         ...
                         */
                            ...
                            // like: 五四运动
                            for (j = num.length(); (cjkidx + j) < chars.length
                            ...
                            /*
                             * @Note: when mached is true, to avoid the start
                             * position problem we have to check the word pool
                             * the same word is exists or not if it exist the
                             * same word to have to clone the word
                             * 
                             * @added: 2014-04-11
                             */
                            // if ( matched && wordPool.contains(w) ) w =
                            // w.clone();

                            IWord wd = null;

                            /*
                             * @Note: when matched is true, num maybe a word
                             * like '五月', yat, this will make it skip the
                             * chinese numeric to arabic logic so find the
                             * matched word that it maybe a single chinese untis
                             * word
                             * 
                             * @added: 2014-06-06
                             */
                            ...
                            if (matched == false && config.CNNUM_TO_ARABIC) {
                                ...
                                String arabic = CNNMFilter.cnNumericToArabic(
                                        num, true) + "";
                        ...
                        if (w != null) {
                            ...
                            if (config.APPEND_CJK_SYN && config.LOAD_CJK_SYN
                                    && w.getSyn() != null) {
                                IWord wd;
                    ...
                    if (config.I_CN_NAME && w.getLength() <= 2
                        ...
                        else if (dic.match(ILexicon.CN_LNAME_ADORN,
                                w.getValue())
                    ...
                    if ((ctrlMask & ISegment.CHECK_CE_MASk) != 0
                         ...
                         */
                        if (!(config.CLEAR_STOPWORD && dic.match(
                                ILexicon.STOP_WORD, cestr))
                    ...
                    // add the syn words to the pool
                    String[] syns = null;
                    if (T == -1 && config.APPEND_CJK_SYN && config.LOAD_CJK_SYN
                            && (syns = w.getSyn()) != null) {
                        IWord wd;
                    ...
                    if (enAfter != null
                            && !(config.CLEAR_STOPWORD && dic.match(
                                    ILexicon.STOP_WORD, enAfter.getValue()))) {
             ...
             */
                ...
                if (ENSCFilter.isEnPunctuation(c)) {
                    ...
                    if (config.CLEAR_STOPWORD
                        ...
                        /*
                         * @added: 2013-12-23. for jcseg-1.9.3 to switch the sub
                         * token ahead of the origin one.
                         */
                        if (sword != null)
                            wordPool.add(w);
            ...
            /*
             * find a content around with pair punctuations. set the pptmaxlen
             * to 0 to close it
             */
                ...
                String str = ((char) c) + "";
                if (!(config.CLEAR_STOPWORD && dic.match(ILexicon.STOP_WORD,
                        str))) {
                    w = new Word(str, IWord.T_PUNCTUATION);
                    w.setPartSpeech(IWord.PUNCTUATION);
                    w.setPosition(pos);
    ...
    private void appendLatinSyn(IWord w) {
        ...
        /*
         * @added 2014-07-07 w maybe EC_MIX_WORD, so check its syn first and
         * make sure it is not a EC_MIX_WORD then check the EN_WORD
         */
        if (w.getSyn() == null)
     ...
     * @param w
     * @param retfw
     *            Wether to return the fword.
     * @param IWord
     *            - the first sub token for the secondary segment.
     */
    public IWord enSecondSeg(IWord w, boolean retfw) {
        ...
        for (j = 1; j < chars.length; j++) {
            ...
            if (_ctype == _TYPE)
                 ...
                 * we create a new IWord and add to the wordPool.
                 */
                if (isb.length() >= config.STOKEN_MIN_LEN) {
                    _str = isb.toString();
                    // check and clear the stopwords
                    if (!(config.CLEAR_STOPWORD && dic.match(
                            ILexicon.STOP_WORD, _str))) {
    ...
    protected IWord[] getNextMatch(char[] chars, int index) {
         ...
         * just return the Word with a value of temp as a unrecognited word.
         */
        if (mList.isEmpty()) {
    ...
    protected IWord nextBasicLatin(int c) throws IOException {
        ...
        ctrlMask &= ~ISegment.START_SS_MASK;   // reset the secondary segment
        // mask.
         ...
         * 
         * set _ctype as the status for the existence of punctuation at the end
         * of the isb cause we need to plus the tcount to avoid the secondary
         * check for words like chenxin+, c+.
         */
        _ctype = 0;
        for (int t = isb.length() - 1; t > 0 && isb.charAt(t) != '%'
        ...
        if (!_check) {
             ...
             */
            if (chkunits
                    && (ENSCFilter.isDigit(__str) || ENSCFilter
                            .isDecimal(__str))) {
                ch = readNext();
                if (dic.match(ILexicon.CJK_UNITS, ((char) ch) + "")) {
                    w = new Word(new String(__str + ((char) ch)),
                            IWord.T_MIXED_WORD);
                    w.setPartSpeech(IWord.NUMERIC_POSPEECH);
                } else
                    pushBack(ch);
        ...
        for (; j < config.MIX_CN_LENGTH && (ch = readNext()) != -1; j++) {
            /*
             * Attension: it is a accident that jcseg works find for we break
             * the loop directly when we meet a whitespace. 1. if a EC word is
     ...
     * simpleSeg and ComplexSeg is different to deal this, so make it a abstract
     * method here.
     * 
     * @param chars
     * @param index
     ...
     * @throws IOException
     */
    protected abstract IChunk getBestCJKChunk(char chars[], int index)
            throws IOException;
