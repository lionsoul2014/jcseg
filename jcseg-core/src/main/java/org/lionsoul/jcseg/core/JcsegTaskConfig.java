    ...
    public void resetFromPropertyFile(String proFile) throws IOException {
        ...
        if (proFile == null) {
            ...
            if (!jcseg_properties) {
                ...
                _report += "3. put jcseg.properties in directory "
        ...
        /* load the mapping from the specified property file. */
            ...
            if (!pro_file.exists())
                throw new IOException("property file [" + proFile
                        + "] not found!");
            lexPro.load(new FileReader(pro_file));
        }
        ...
        if (lexDirs == null)
            throw new IOException(
                    "lexicon.path property not find in jcseg.properties file!!!");
        ...
        if (lexPro.getProperty("jcseg.mixcnlen") != null)
            MIX_CN_LENGTH = Integer.parseInt(lexPro
                    .getProperty("jcseg.mixcnlen"));
        ...
        if (lexPro.getProperty("jcseg.cnmaxlnadron") != null)
            MAX_CN_LNADRON = Integer.parseInt(lexPro
                    .getProperty("jcseg.cnmaxlnadron"));
        if (lexPro.getProperty("jcseg.nsthreshold") != null)
            NAME_SINGLE_THRESHOLD = Integer.parseInt(lexPro
                    .getProperty("jcseg.nsthreshold"));
        if (lexPro.getProperty("jcseg.pptmaxlen") != null)
            PPT_MAX_LENGTH = Integer.parseInt(lexPro
                    .getProperty("jcseg.pptmaxlen"));
        ...
        if (lexPro.getProperty("jcseg.stokenminlen") != null)
            STOKEN_MIN_LEN = Integer.parseInt(lexPro
                    .getProperty("jcseg.stokenminlen"));
