...
public class DicMerge {
    ...
    public static void main(String[] args) {
        ...
        if (args.length < 2) {
            System.out
                    .println("Usage: java -jar jcseg-dicmerge.jar {dstdir} {srcdir1} {srcdir2} ...");
            System.out
                    .println("{dstdir}: directory to store all the merge dictionary.");
        ...
        if (!dstDir.exists()) {
            System.out.println("error: srcDir {" + args[0]
        ...
        for (int j = 1; j < args.length; j++) {
            ...
            if (!f.exists()) {
                System.out.println("error: dstDir {" + args[j]
                        + "} is not a valid directory.");
                System.exit(0);
            }
        }
        ...
        try {
            ...
            for (int j = 0; j < files.length; j++) {
                ...
                // files[j].getName()+"... ");
                System.out.format("+-Merging file %20s ... ",
                        files[j].getName());
                dstList.clear();
                dstList.add(files[j]);
                ...
                int ct = merge(dstFiles, new File(dstDir.getAbsolutePath()
                        + "/" + files[j].getName()));
                if (ct > 0) {
                    counter++;
