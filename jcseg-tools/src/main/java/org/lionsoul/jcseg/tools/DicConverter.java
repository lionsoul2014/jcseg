...
public class DicConverter {
    ...
    public static void main(String[] args) {
        if (args.length != 3) {
            System.out
                    .println("Usage: java -jar jcseg-dicst.jar {tpy} {src} {dst}");
            System.out.println("{typ}: convert type.");
            System.out
                    .println("       0 for convert simplified to traditional.");
            System.out
                    .println("       1 for convert traditional to simplified.");
        ...
        if (!srcDir.exists()) {
            System.out.println("error: srcDir {" + args[1]
        ...
        if (!dstDir.exists()) {
            System.out.println("error: dstDir {" + args[2]
                    + "} is not a valid directory.");
        }
        ...
        try {
            ...
            for (int j = 0; j < files.length; j++) {
                System.out.format("+-Converting file %20s ... ",
                        files[j].getName());
                if (convert(files[j], new File(dstDir.getAbsolutePath() + "/"
                        + files[j].getName()), cvt)) {
            ...
            System.out.println("+Done, Total [" + files.length
                    + "], successfull convert [" + success + "]");
